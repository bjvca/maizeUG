...
## merge in treatments
treats <- read.csv("/home/bjvca/data/projects/digital green/sampling/sampling_list_pub.csv")[c("HHID","IVR","sms")]
names(treats) <- c("hhid","ivr","sms")
treats$femhead <- FALSE
###merge in treatments for FHs - did they receive sms messages?
treats_FH <- read.csv("/home/bjvca/data/projects/digital green/sampling/femhead_list_pub.csv")[c("HHID","IVR")]
...
library(foreign)
dta <- merge(dta,read.csv("/home/bjvca/data/projects/digital green/endline/data/call_log_pub.csv"), by="hhid", all.x=T)
dta$called <- !is.na(dta$called)


dta <- merge(dta, read.csv("/home/bjvca/data/projects/digital green/endline/data/sms_log_pub.csv"), by.x="hhid", by.y="HHID", all.x=T)
