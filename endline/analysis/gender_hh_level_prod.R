...
res_prod_hh <- array(NA, c(8,4,7)) 
rownames(res_decision_b) <- c("prod","","area","","yield_better","","yield","")
...
########################*************************** HH level *****************************############################
dta_cpy <- dta
dta <-  trim("prod_hh_mean",dta_cpy,.05)

res_prod_hh[1,1,h] <- ifelse(h ==1, wtd.mean(dta$prod_hh_mean[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.mean(dta$prod_hh_mean[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.mean(dta$prod_hh_mean[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.mean(dta$prod_hh_mean[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.mean(dta$prod_hh_mean[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[2,1,h] <- ifelse(h ==1, wtd.sd(dta$prod_hh_mean[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.sd(dta$prod_hh_mean[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.sd(dta$prod_hh_mean[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.sd(dta$prod_hh_mean[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.sd(dta$prod_hh_mean[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[1,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("prod_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,1],summary(lm(as.formula(paste(paste("prod_hh_mean",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,1])
res_prod_hh[2,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("prod_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,2],summary(lm(as.formula(paste(paste("prod_hh_mean",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,2])
res_prod_hh[1,3,h] <- ifelse(totrep >0, RI("prod_hh_mean",treatment , ctrls,w_int="weights", dta, nr_repl = totrep, h),ifelse(is.null(ctrls),summary(lm(as.formula(paste("prod_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,4],summary(lm(as.formula(paste(paste("prod_hh_mean",treatment, sep="~"),ctrls, sep="+")),weights=weights,data=dta))$coefficients[2,4]))
res_prod_hh[2,3,h] <- nobs(lm(as.formula(paste("prod_hh_mean",treatment, sep="~")) ,data=dta))
dta <-  trim("area_hh_mean",dta_cpy,.05)
res_prod_hh[3,1,h] <- ifelse(h ==1, wtd.mean(dta$area_hh_mean[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.mean(dta$area_hh_mean[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.mean(dta$area_hh_mean[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.mean(dta$area_hh_mean[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.mean(dta$area_hh_mean[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[4,1,h] <- ifelse(h ==1, wtd.sd(dta$area_hh_mean[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.sd(dta$area_hh_mean[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.sd(dta$area_hh_mean[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.sd(dta$area_hh_mean[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.sd(dta$area_hh_mean[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[3,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("area_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,1],summary(lm(as.formula(paste(paste("area_hh_mean",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,1])
res_prod_hh[4,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("area_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,2],summary(lm(as.formula(paste(paste("area_hh_mean",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,2])
res_prod_hh[3,3,h] <- ifelse(totrep >0, RI("area_hh_mean",treatment , ctrls,w_int="weights", dta, nr_repl = totrep, h),ifelse(is.null(ctrls),summary(lm(as.formula(paste("area_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,4],summary(lm(as.formula(paste(paste("area_hh_mean",treatment, sep="~"),ctrls, sep="+")),weights=weights,data=dta))$coefficients[2,4]))
res_prod_hh[4,3,h] <- nobs(lm(as.formula(paste("area_hh_mean",treatment, sep="~")) ,data=dta))

res_prod_hh[5,1,h] <- ifelse(h ==1, wtd.mean(dta$yield_better[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.mean(dta$yield_better[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.mean(dta$yield_better[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.mean(dta$yield_better[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.mean(dta$yield_better[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[6,1,h] <- ifelse(h ==1, wtd.sd(dta$yield_better[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.sd(dta$yield_better[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.sd(dta$yield_better[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.sd(dta$yield_better[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.sd(dta$yield_better[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[5,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("yield_better",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,1],summary(lm(as.formula(paste(paste("yield_better",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,1])
res_prod_hh[6,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("yield_better",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,2],summary(lm(as.formula(paste(paste("yield_better",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,2])
res_prod_hh[5,3,h] <- ifelse(totrep >0, RI("yield_better",treatment , ctrls,w_int="weights", dta, nr_repl = totrep, h),ifelse(is.null(ctrls),summary(lm(as.formula(paste("yield_better",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,4],summary(lm(as.formula(paste(paste("yield_better",treatment, sep="~"),ctrls, sep="+")),weights=weights,data=dta))$coefficients[2,4]))
res_prod_hh[6,3,h] <- nobs(lm(as.formula(paste("yield_better",treatment, sep="~")) ,data=dta))
dta <-  trim("yield_hh_mean",dta_cpy,.05)
res_prod_hh[7,1,h] <- ifelse(h ==1, wtd.mean(dta$yield_hh_mean[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.mean(dta$yield_hh_mean[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.mean(dta$yield_hh_mean[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.mean(dta$yield_hh_mean[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.mean(dta$yield_hh_mean[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[8,1,h] <- ifelse(h ==1, wtd.sd(dta$yield_hh_mean[dta$recipient == "male" | dta$recipient == "female"], dta$weights[dta$recipient == "male" | dta$recipient == "female"], na.rm=T),ifelse(h %in% c(2,3,4), wtd.sd(dta$yield_hh_mean[dta$recipient == "male"],dta$weights[dta$recipient == "male"], na.rm=T), ifelse( h == 5, wtd.sd(dta$yield_hh_mean[dta$messenger == "male" | dta$messenger == "female"],dta$weights[dta$messenger == "male" | dta$messenger == "female"], na.rm=T), ifelse(h == 6, wtd.sd(dta$yield_hh_mean[dta$messenger == "male" ],dta$weights[dta$messenger == "male" ], na.rm=T), wtd.sd(dta$yield_hh_mean[dta$messenger != dta$recipient],dta$weights[dta$messenger != dta$recipient], na.rm=T)))))
res_prod_hh[7,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("yield_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,1],summary(lm(as.formula(paste(paste("yield_hh_mean",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,1])
res_prod_hh[8,2,h] <- ifelse(is.null(ctrls),summary(lm(as.formula(paste("yield_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,2],summary(lm(as.formula(paste(paste("yield_hh_mean",treatment, sep="~"),ctrls, sep="+")) ,weights=weights,data=dta))$coefficients[2,2])
res_prod_hh[7,3,h] <- ifelse(totrep >0, RI("yield_hh_mean",treatment , ctrls,w_int="weights", dta, nr_repl = totrep, h),ifelse(is.null(ctrls),summary(lm(as.formula(paste("yield_hh_mean",treatment, sep="~")) ,weights=weights,data=dta))$coefficients[2,4],summary(lm(as.formula(paste(paste("yield_hh_mean",treatment, sep="~"),ctrls, sep="+")),weights=weights,data=dta))$coefficients[2,4]))
res_prod_hh[8,3,h] <- nobs(lm(as.formula(paste("yield_hh_mean",treatment, sep="~")) ,data=dta))
...
plot_ag[4,1] <- "production"
plot_ag[3,1] <- "area"
plot_ag[2,1] <- "yield better?"
plot_ag[1,1] <- "productivity"


plot_ag[4,2] <- res_prod_hh[1,2,h]/res_prod_hh[1,1,h]
plot_ag[3,2] <- res_prod_hh[3,2,h]/res_prod_hh[3,1,h]
plot_ag[2,2] <- res_prod_hh[5,2,h]/res_prod_hh[5,1,h]
plot_ag[1,2] <- res_prod_hh[7,2,h]/res_prod_hh[7,1,h] 

plot_ag[4,3] <- (res_prod_hh[1,2,h] - sig*res_prod_hh[1,2,h])/res_prod_hh[1,1,h]
plot_ag[3,3] <- (res_prod_hh[3,2,h] - sig*res_prod_hh[3,2,h])/res_prod_hh[3,1,h]
plot_ag[2,3] <- (res_prod_hh[5,2,h] - sig*res_prod_hh[5,2,h])/res_prod_hh[5,1,h]
plot_ag[1,3] <- (res_prod_hh[7,2,h] - sig*res_prod_hh[7,2,h])/res_prod_hh[7,1,h]

plot_ag[4,4] <- (res_prod_hh[1,2,h] + sig*res_prod_hh[1,2,h])/res_prod_hh[1,1,h]
plot_ag[3,4] <- (res_prod_hh[3,2,h] + sig*res_prod_hh[3,2,h])/res_prod_hh[3,1,h]
plot_ag[2,4] <- (res_prod_hh[5,2,h] + sig*res_prod_hh[5,2,h])/res_prod_hh[5,1,h]
plot_ag[1,4] <- (res_prod_hh[7,2,h] + sig*res_prod_hh[7,2,h])/res_prod_hh[7,1,h]

plot_ag$x <- factor(plot_ag$x, levels=plot_ag$x)


pdf(paste(paste("/home/bjvca/data/projects/digital green/endline/results/prodplot",h,sep="_"),".pdf", sep=""))
credplot.gg(plot_ag,'%')
dev.off()
