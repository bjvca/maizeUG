...
d_plot$x <- rep(c("","knowledge index","knows about spacing", "knows about combining","knows about weeding","knows about FAW", " ","decision index","decision to plant maze","decision on timing of planting","decision on spacing & seed rate","decision on how to fight striga","decision on weeding","  ","adoption index","timely planting","optimal spacing & seed rate","recommended striga weeding","recommended timing of weeding", "   ","input use index","DAP","UREA","Organic","Hybrid seed", "OPV","    ", "outcome index","yield improved","yield","sold","quantity sold","     "), each=3)
d_plot$grp <- rep(c("woman outcome","joint outcome","man outcome"), times=33)
d_plot$grp <-  factor(d_plot$grp , levels=c("woman outcome","joint outcome","man outcome"))
d_plot$x <-  factor(d_plot$x, levels=rev(c("","knowledge index","knows about spacing", "knows about combining","knows about weeding","knows about FAW", " ","decision index","decision to plant maze","decision on timing of planting","decision on spacing & seed rate","decision on how to fight striga","decision on weeding","  ","adoption index","timely planting","optimal spacing & seed rate","recommended striga weeding","recommended timing of weeding","   ", "input use index","DAP","UREA","Organic","Hybrid seed", "OPV","    ", "outcome index","yield improved","yield","sold","quantity sold","     ")))

bold.labels <- ifelse(levels(d_plot$x) %in% c("knowledge index","decision index","adoption index","input use index","outcome index"), yes = "bold", no = "plain")
